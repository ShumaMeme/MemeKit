            ...
            if (not force_simple) and total_size > 0 and accept_ranges and self.threads > 1:
                if self._download_with_threads(total_size):
                    ...
                    if not need_retry:
                        ...
                        self.result_ready.emit(self.dest)
                        return
                ...
                for chunk in response.iter_content(chunk_size=64 * 1024):
                    if self.is_canceled:
                        if os.path.exists(self.dest):
                            os.remove(self.dest)
                        self.result_ready.emit("CANCELED")
                        return
                    if chunk:
                        file.write(chunk)
...
class FirmwareTab(QWidget):
    ...
    def _start_load(self):
        ...
        try:
            ...
            src = ""
        self._loader_worker = _FirmwareListLoader(src, parent=self)
    ...
    def _on_download(self, name: str, url: str):
        ...
        # 创建下载工作器
        worker = DownloadWorker(url, path, parent=self)
        
        # 存储当前下载信息
        self.current_download = {'worker': worker, 'name': name}
    ...
    def _force_cleanup_download(self):
        ...
        if worker.isRunning():
            try:
                ...
                try:
                    worker.terminate(); worker.wait(100)
                except Exception:
                    pass
    ...
    def cleanup(self):
        ...
        # 停止加载线程
        try:
            t = getattr(self, '_loader_worker', None)
...
class _FirmwareListLoader(QThread):
    loaded = Signal(list)
    error = Signal(str)
